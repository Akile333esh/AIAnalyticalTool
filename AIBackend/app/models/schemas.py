...
class RAGTableMetadata(BaseModel):
    schema: str = Field(..., description="SQL schema name, e.g. 'dbo'")
...
class SQLGenRequest(BaseModel):
    ...
    natural_language: user question / instruction
    time_range: optional high-level time window (e.g. 'last_7_days', '30d', 'custom')
    metric_type: cpu | memory | disk | mixed, etc.
    filters: arbitrary JSON filter structure (device names, instances, etc.)
    metadata: RAG metadata describing available tables/columns/joins/examples
    """
